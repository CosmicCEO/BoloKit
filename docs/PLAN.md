...
| Wave 6 | Networking — summary row; see 6.0–6.5 below for the D32 sub-wave split (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0 of 6 sub-waves complete (PARITY PASS); 6.1 pre-brief GO'd, 6.2–6.5 not started |
| Wave 6.0 | Wire codec — `CL*`/`SR*` structs, `CLUpdate`/preambles, all three encodings (raw BE float, 1/256 fixed-point, 8-bit brads), derived byte-exact from the C oracle | ✅ Complete — PARITY PASS, `96704cd`+`5c5e47a` (FWIDTH-double finding independently re-derived by PARITY) — 345 tests total (+16) |
| Wave 6.1 | Tick orchestrator — `runclient()`/`runserver()` | 🟩 Pre-brief GO issued 2026-09-02 (see `docs/AGENT_NOTES.md`) — no pre-brief written yet |
