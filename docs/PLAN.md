...
| Wave 6 | Networking — summary row; see 6.0–6.6 below for the D32 sub-wave split plus D38's 6.6 addition (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0/6.1/6.2/6.3 complete; D39 fix required before Wave 6.4's pre-brief GO; Wave 6.6 pre-brief GO'd 2026-09-03, recommended to run before 6.4 (D38); 6.5 forward-planned (D32/D36) but not started |
...
| Wave 6.6 | **New 2026-09-03 (D38, Wave 6.3 PARITY note):** the ~19 unassigned `recvcl*` server TCP-receive handlers (`recvcldamage`, `recvclbuildroad`/`buildwall`/`buildboat`/`buildpill`, `recvclrepairpill`, `recvclplacemine`, `recvclsmallboom`/`superboom`, `recvclrefuel`, `recvclhittank`, `recvclsendmesg`, `recvcldropboat`/`droppills`/`dropmine`, `recvcltouch`, `recvclgrabtile`/`grabtrees`) — same shape as Wave 6.2/6.3's session logic (pure decision/mutation core, split from the receive mechanism per the 6.3 precedent), covering every `CL*` opcode except `CLSetAlliance` (already done, Wave 6.3). | 🟨 Pre-brief GO'd 2026-09-03 — unblocked, no dependency on 6.4's transport work; recommended to run before 6.4 so 6.4's dispatch wiring has real handlers to call for every `CL*` opcode, not just `recvClSetAlliance` (Jerod's call if he wants it reordered) |
