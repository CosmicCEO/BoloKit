...
| Wave 6.4a | **Split from Wave 6.4 per D43.** Client-side transport: `joinClient(...)`-shaped async join handshake against `NWConnection` (byte-exact per D31/D33); `dgramclient()`'s post-decode CLUpdate-application logic (`PlayerState` field application, shell/explosion list rebuild, bounded dead-reckoning per D44); fog-of-war/sound callbacks simplified per this port's established no-op precedent. **Per D45 (2026-09-03), also owns `joinclient()`'s back half (`client.c:690-750`) — applying a received `BoloPreamble` to `GameState` (player index, roster init, pause/gametype, spawn) — mis-cited in the original pre-brief as covered by server-side functions that don't actually cover it; plus the persistent `NWConnection` receive loops (UDP → `applyRemotePlayerUpdate`, TCP → dispatch decoded `SR*` opcodes to the already-shipped `recvSr*` handlers) that fell through D43's split without being explicitly assigned to either sub-wave.** | 🟨 **Not closed — real scope gap found by PARITY audit (`60d5059`).** `e4ca245`+`970a1d8` (502 tests) covers the join-handshake wire steps and `dgramclient` application cleanly, PARITY found no fidelity divergence in code that exists — but the wave isn't finished. D45 extends scope, coding GO reissued for the extension only. |
