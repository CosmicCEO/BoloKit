...
| Wave 6 | Networking — summary row; see 6.0–6.6 below for the D32 sub-wave split plus D38's 6.6 addition (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0/6.1/6.2/6.3 complete; D39 closed (`029c8fc`, re-audit `b1efc12` clean); Wave 6.6 coded (`ebb8fe4`, 487 tests), awaiting PARITY audit; Wave 6.4 unblocked, pre-brief GO to follow once 6.6's audit clears per D38; 6.5 forward-planned (D32/D36) but not started |
...
| Wave 6.4 | Transport + join handshake — Network.framework + async/await (D31); builds on `Buf.swift`'s existing `sendbuf`/`recvbuf`/`cntlsend`/`cntlrecv`. **Per D36 (2026-09-03), also owns `dgramclient()`'s post-decode CLUpdate-application logic (split pure state-application from the receive mechanism, mirroring 6.3's `joinplayerserver()` split) and the server-side tracker reachability echo (`player == 255` sentinel).** | ⬜ Unblocked (D39 closed 2026-09-03) but not started — sequenced after Wave 6.6 per D38, which is currently coding |
...
| Wave 6.6 | **New 2026-09-03 (D38, Wave 6.3 PARITY note):** the ~19 unassigned `recvcl*` server TCP-receive handlers (`recvcldamage`, `recvclbuildroad`/`buildwall`/`buildboat`/`buildpill`, `recvclrepairpill`, `recvclplacemine`, `recvclsmallboom`/`superboom`, `recvclrefuel`, `recvclhittank`, `recvclsendmesg`, `recvcldropboat`/`droppills`/`dropmine`, `recvcltouch`, `recvclgrabtile`/`grabtrees`) — same shape as Wave 6.2/6.3's session logic (pure decision/mutation core, split from the receive mechanism per the 6.3 precedent), covering every `CL*` opcode except `CLSetAlliance` (already done, Wave 6.3). | ✅ Coded — `ebb8fe4` (487 tests, +40), pending PARITY audit. Corrected two pre-brief claims during coding (disclosed, not silently absorbed): `recvClTouch` calls `explosionAt` directly, not `touchTile` (10 new `explosionAt` trigger sites, not 9); `explosionAt`/`superboomAt` have no broadcast-callback of their own, worked around locally, plus a confirmed asymmetry between the two (attribution and gating differ). D40 applied to `recvclbuildroad`. Running before Wave 6.4 per D38. |
