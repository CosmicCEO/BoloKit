...
| Wave 6.4 | Transport + join handshake — Network.framework + async/await (D31); builds on `Buf.swift`'s existing `sendbuf`/`recvbuf`/`cntlsend`/`cntlrecv`. **Per D36 (2026-09-03), also owns `dgramclient()`'s post-decode CLUpdate-application logic (split pure state-application from the receive mechanism, mirroring 6.3's `joinplayerserver()` split) and the server-side tracker reachability echo (`player == 255` sentinel).** | 🟨 Pre-brief GO'd 2026-09-03 — both gates (D39, Wave 6.6) clear |
