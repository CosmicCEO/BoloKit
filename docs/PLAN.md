...
| **Q21** | `heatPill` (Wave 5.3a, `ShellTick.swift`) resets `Pill.counter` on damage, but `Pill.coolCounter`'s own doc comment identifies `server.pills[i].counter` — the field `recvcldamage()` (`server.c:2804-2846`) actually resets — as *its* C analog, not `Pill.counter`'s. Discovered incidentally while porting `recvSrDamage` (Wave 6.2), which doesn't call `heatPill` and is correct regardless of how this resolves. If the mapping read is right, `heatPill` has reset the wrong field since Wave 5.3a — a real, narrow, pre-existing fidelity question, not yet independently confirmed. Awaiting PARITY's field-mapping re-derivation against `server.c:2804-2846`/`client.c:1540-1577` as part of the Wave 6.2 audit before ruling fix-now vs. tracked item (Wave 5.9-style). | Wave 6.2 audit's outcome decides the ruling, not code-blocking itself | Medium |
