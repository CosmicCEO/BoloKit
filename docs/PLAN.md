...
| Wave 5.9 | **New 2026-09-02 (Wave 6.1 finding):** wire the mine-cascade injection points (`onMineExplosion`/`onSuperboomTerrain`/`onDropPills`) into their actual trigger sites — `enterTile`, `grabTile`, `tankMoveTick`'s dead-tumble, `smallboom`/`superboom` — with correct causer-player attribution (`state.localPlayer`, uniformly, per the pre-brief). Every trigger site has left these as documented no-ops since Wave 5.2b; the engine functions themselves (`explosionAt`/`superboomAt`/`chain`/`flood`) already shipped in 5.5a. This is unfinished Phase 3 port work (behaviour-preserving, design already settled by 5.5a/D22), not a Phase 5 fidelity fix and not Wave 6 networking — numbered 5.9 rather than folded into 6.x. Confirmed independent of Wave 6.2–6.5 (which only apply already-decided values, never re-trigger cascades), so it does not block networking's sequence. **Now includes the periodic corpse-explosion `killPointBuilder` gap found in the pre-brief (ruled in-scope by PLANNER — same literal C function this wave already owns, not a separate tracked item).** Running as a scoped agent on its own branch/worktree (`wave-5.9-mine-cascade`) per `docs/WAVE59_BOOTSTRAP.md`, in parallel with the main sequence. | 🟨 **Coded** — `d9116a9` (pre-brief) → `d3d654e` (coding, 453 tests, +6) → `7cb409f` (completion report), all on `wave-5.9-mine-cascade`. Implementation matches D41's ruling and the in-scope corpse-explosion fix exactly. Reviewed by PLANNER, PARITY audit requested; not yet merged to `main` — merge happens only after PARITY PASS, per `docs/WAVE59_BOOTSTRAP.md`'s own process. |

| Wave 6 | Networking — summary row; see 6.0–6.6 below for the D32 sub-wave split plus D38's 6.6 addition (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0/6.1/6.2/6.3/6.6 complete, all PARITY PASS; D39 closed; Wave 6.4 split into 6.4a (client, pre-brief GO'd)/6.4b (host, blocked on 6.4a) per D43; 6.5 forward-planned (D32/D36) but not started. Wave 5.9 (independent of this sequence) is now in progress under its own session — the earlier parallel-agent worktree attempt hit an API overload incident (not a scope/design problem) but a retry/pickup is coding now per `docs/WAVE59_BOOTSTRAP.md`. |
...
| Wave 6.4a | **Split from Wave 6.4 per D43.** Client-side transport: `joinClient(...)`-shaped async join handshake against `NWConnection` (reusing Wave 6.3's `JoinPreamble`/`BoloPreamble` types and `evaluateJoinRequest`/`applyJoin`/`assembleBoloPreamble` pure logic, byte-exact per D31/D33); `dgramclient()`'s post-decode CLUpdate-application logic (`PlayerState` field application, shell/explosion list rebuild, bounded dead-reckoning per D44); fog-of-war/sound callbacks simplified per this port's established no-op precedent. Sequenced first — more directly differentially-testable (Wave 6.0's `clupdate_decode_oracle`) than 6.4b's server-side half. | ⬜ Pre-brief GO'd 2026-09-03 (as part of Wave 6.4's original pre-brief, `9c3383d`) — not yet started |
| Wave 6.4b | **Split from Wave 6.4 per D43.** Host-side transport: `NWListener`-based async accept loop wired to Wave 6.3's join-decision functions; `dgramserver()`'s dedup/relay pure decision logic (Finding 2, confirmed in-scope by D43) — packet sanity/player-range check, `dgramaddr` port update, `isNewerSeq`-based dedup (Wave 6.0), tank-position apply, cross-player relay; the tracker reachability echo at `dgramserver()`'s occurrence only (`registerserver()`'s occurrence deferred to Wave 6.5 per the pre-brief's proposed split, confirmed). | ⬜ Blocked on 6.4a completing — not yet started |
