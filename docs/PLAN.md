...
| Wave 6 | Networking — summary row; see 6.0–6.6 below for the D32 sub-wave split plus D38's 6.6 addition (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0/6.1/6.2/6.3/6.6 complete, all PARITY PASS; D39 closed. Wave 6.4 split into 6.4a/6.4b per D43; 6.4a extended per D45 (`joinclient()` back half, `decodeBMap`, persistent sessions), still not closed — D46 requires one small fix (`baseControlThreshold`) before it can close; 6.4b blocked until then. 6.5 forward-planned (D32/D36) but not started. **Wave 5.9 (independent of this sequence) is complete — PARITY PASS, merged to `main`.** |
