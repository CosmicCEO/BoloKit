...
| Wave 6.4c | **New 2026-09-03 (D50 — Wave 6.4b completion report's three disclosed open issues, PARITY-confirmed real).** Wire `NWListener(using: .udp)` to drive `DgramServerRelay.swift`'s existing decision core against real datagrams; derive `dgramaddr` from the joining TCP connection's real address at join (`server.c:844`) instead of a zeroed placeholder; add `onShouldBroadcastDropPill` to `MineChain.swift`'s `droppills()`/`dr()` (Wave 5.5a) and wire `SRDropPill`, mirroring 6.4b's G-2 pattern. | 🟨 **Coded, tested, committed — awaiting PARITY audit.** Pre-brief `091c364` (D51/D52 ruled) → `5fdb1bc`+`b3d6c8a` (Implementer: full implementation + completion report, 571 tests, +11, no coverage lost; all three D50 items fully wired, no new open questions or scope gaps disclosed this time, unlike 6.4b's three carry-overs). Planner reviewed the completion report and confirms D50/D51/D52 fully addressed — PARITY formally activated. |
