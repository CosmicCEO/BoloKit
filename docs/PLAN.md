...
| Wave 6.6 | **New 2026-09-03 (D38, Wave 6.3 PARITY note):** the ~19 unassigned `recvcl*` server TCP-receive handlers (`recvcldamage`, `recvclbuildroad`/`buildwall`/`buildboat`/`buildpill`, `recvclrepairpill`, `recvclplacemine`, `recvclsmallboom`/`superboom`, `recvclrefuel`, `recvclhittank`, `recvclsendmesg`, `recvcldropboat`/`droppills`/`dropmine`, `recvcltouch`, `recvclgrabtile`/`grabtrees`) — same shape as Wave 6.2/6.3's session logic (pure decision/mutation core, split from the receive mechanism per the 6.3 precedent), covering every `CL*` opcode except `CLSetAlliance` (already done, Wave 6.3). Pre-brief found: 4 of the 19 are thin wrappers around already-shipped Wave 5.3a/5.5a engine calls (`recvcldamage`/`recvcltouch`/`recvclsmallboom`/`recvclsuperboom`); 9 give clean new `explosionAt`/`superboomAt` trigger sites independent of Wave 5.9's still-open gap; `recvclsendmesg` needs no state-mutating function at all (Wave 6.2 precedent), `recvclhittank` is a near-trivial bounds-check-and-relay. | 🟨 **Coding GO'd 2026-09-03** — pre-brief (`a5d518d`) reviewed clean; its one open question (`recvclbuildroad`'s tautological tree-cost check) ruled by D40 (replicate bug-for-bug). Running before Wave 6.4 per D38. |
