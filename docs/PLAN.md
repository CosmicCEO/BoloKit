...
| **Q20** | If DEEPDIVE1 finds WinBolo's wire format matches the 0.99.7bv oracle, could you skip Wave 6 server/transport recoding entirely and just run WinBolo's server unmodified as a separate process (your Swift client talks to it over sockets)? Licensing looks favorable — an unmodified GPL binary run as a separate process communicating over a documented protocol is "mere aggregation," not linking, so it wouldn't taint BoloKit's MIT license the way adapting WinBolo's code would (Q19 options 2-4). The open risk is fidelity, not license: it's not established whether original Bolo's server is authoritative for tank physics (in which case WinBolo's already-documented deviations — wall friction, tank deceleration, boat-to-land transition, mine self-damage, pillbox range — would govern every networked game) or a thin arbiter for shared state only (pill contests, tree growth, spawn selection, base repair — the D27-class shared per-tick fields), with each client computing its own tank's physics locally regardless of server choice. Wave 5's merge of `tanklocallogic`/`enter()` with the `recvcl*`/`recvsr*` handlers into one Swift tick function is suggestive of the latter but not conclusive. Also not free even in the best case: the Swift client still needs a full, correct implementation of both directions of the wire protocol to talk to WinBolo's server at all — this would skip server session logic and transport plumbing, not client-side codec work. **Held pending DEEPDIVE1's wire-format finding; not yet assigned to a session.** | Q16/Q19 ruling | Medium |
