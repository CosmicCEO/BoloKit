...
| Wave 6 | Networking — summary row; see 6.0–6.5 below for the D32 sub-wave split (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0/6.1/6.2 complete; 6.3 coding GO'd 2026-09-03; 6.4/6.5 forward-planned (D36) but not started |
...
| Wave 6.3 | Server session logic — join/kick/ban/alliance; **now also the three preamble structs** (`JOIN_Preamble`/`BOLO_Preamble`/`TRACKER_Preamble`) reassigned from Wave 6.0's corrected row above | 🟨 Coding GO'd 2026-09-03 (6.2 closed) — pre-brief `ccb4481` stands as reviewed, no re-review needed |
