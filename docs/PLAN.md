...
| Wave 6 | Networking — summary row; see 6.0–6.5 below for the D32 sub-wave split (UI split out to its own phase per IMPLEMENTER's 2026-09-02 scope survey — see AGENT_NOTES.md) | 🟨 In progress — 6.0 complete; 6.1 audited, 2 findings, fix required before close (D35); 6.2/6.3 pre-briefs reviewed and banked (Jerod requested all three pre-briefs ahead of the normal cadence); 6.4/6.5 not started |
...
| Wave 6.1 | Tick orchestrator — `runclient()`/`runserver()`; new pause/timelimit/domination-basecontrol state machines | 🟨 PARITY-audited, 2 findings (`c1cddee`) — **D35: fix both before close**, IMPLEMENTER GO'd directly (361 tests, +16 — will grow with the fix) |
| Wave 6.2 | ~33 `recvsr*` client broadcast handlers (apply-given-value state application, a distinct category from Wave 5's authoritative tick functions — not a thin wiring layer) | 🟨 Pre-brief reviewed and banked 2026-09-02 — coding GO follows once 6.1 closes, per D32's order |
...
| Wave 6.4 | Transport + join handshake — Network.framework + async/await (D31); builds on `Buf.swift`'s existing `sendbuf`/`recvbuf`/`cntlsend`/`cntlrecv` | ⬜ Not started — blocked on 6.0–6.3 |
| Wave 6.5 | Tracker/NAT-PMP — permissive alternative to `TCMPortMapper` per `README.md:42-45`'s existing GPLv3-avoidance commitment (D32) | ⬜ Not started — deferrable per D4; blocked on 6.4 |
