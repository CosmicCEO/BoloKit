...
**Two-Mac run, 2026-09-22, build `v151-test5 9a58453`** (both Macs; host Mac A 192.168.86.176, guest Mac B): ran `docs/TEST_TWO_MAC_FOG.md` in full — results and handwritten notes in `docs/Test Result Two-Mac Fog Test Script (v1.5.1).pdf`, printed script in `docs/Two-Mac Fog Test Script (v1.5.1).docx`. Mostly pass. **Live-confirmed fixed and closed:** [#79](https://github.com/CosmicCEO/BoloKit/issues/79) (label fog-gate, step 3 check 9 PASS — also confirmed in code earlier the same day, PR [#119](https://github.com/CosmicCEO/BoloKit/pull/119)), [#107](https://github.com/CosmicCEO/BoloKit/issues/107) (host-quit disconnect message, step 13 PASS). **[#77](https://github.com/CosmicCEO/BoloKit/issues/77) narrowed further:** its "own mine always visible at range" claim was actually the intended sticky-reveal design (step 3 check 4 PASS); only the builder-laid-mine render-refresh gap remains open. **New issue filed:** [#120](https://github.com/CosmicCEO/BoloKit/issues/120), alliance breaking doesn't re-fog the vision it granted (tank sprite disappears, revealed terrain doesn't recede). **New detail added to existing issues:** [#113](https://github.com/CosmicCEO/BoloKit/issues/113) (rejoin — control loss now pinned to reaching the map border, not a fixed distance; a new-player-name rejoin still spawns at the guest's frozen position, contradicting `docs/RELEASE_NOTES_v1.5.1.md`'s prior "worked in testing" note), [#114](https://github.com/CosmicCEO/BoloKit/issues/114) (shells — angle-dependent visibility gaps, reproduces even with Hidden Mines off), [#93](https://github.com/CosmicCEO/BoloKit/issues/93) (first-join-fails-second-succeeds, reproduced again). **Not re-tested this run:** #105 (the script's mine checks used a stationary Shift-laid mine, not the continuous-lay-while-moving scenario #105 fixes) — stays open per the note below.

**Still genuinely open in v1.5.1** (real regressions, not investigation-blocked): builder-laid mine not visible to the host until the tank moves, a render-refresh gap ([#77](https://github.com/CosmicCEO/BoloKit/issues/77), narrowed to just this), shells not always visible for their whole flight, now with angle data pointing at a possible heading-column mapping bug ([#114](https://github.com/CosmicCEO/BoloKit/issues/114)), alliance-break vision leak ([#120](https://github.com/CosmicCEO/BoloKit/issues/120)). Fix them, confirm with `swift test` plus a live two-Mac run, then cut the `v1.5.1` tag from `docs/RELEASE_NOTES_v1.5.1.md`.

**[#105](https://github.com/CosmicCEO/BoloKit/issues/105) fixed on a branch, not merged (2026-09-21):** a host-simulated guest's key-down mine landed on the tile it already occupied, and `remoteLastTankPosition` was still one tile behind, so the next tick detonated it. `9a58453` on `claude/coding-session-a9s35l` stamps the tank's current position when that drop is on its own tile. A snap onto a mine that was already there still detonates. Tests: `dispatchDropMineUnderAHostSimulatedTankDoesNotDetonateOnTheNextTick`, `hostSimulatedTankAlreadyOnAMineStillDetonatesWhenTheEntryStampIsBehind` (filtered `swift test` only; full suite not run). Desktop build for the two-Mac check: `Bolo 2026 (v151-test5 9a58453).app`. The 2026-09-22 run above used this exact build but didn't re-exercise the continuous-lay-while-moving scenario this fix targets — issue stays open until that specific check runs. Pull requests [#95](https://github.com/CosmicCEO/BoloKit/pull/95) and [#109](https://github.com/CosmicCEO/BoloKit/pull/109) were closed as superseded by #117.
...
**Next step:** confirm #105 with the continuous-lay-while-moving scenario on the two-Mac build above, then merge `9a58453`. After that, fix the three still-open v1.5.1 issues (builder-mine render-refresh — #77; shells — #114; alliance-break vision leak — #120). #113 and #89 stay in v1.5.2. Run `swift test` against `main` before the `v1.5.1` tag, then update this file's "Shipped" section.
