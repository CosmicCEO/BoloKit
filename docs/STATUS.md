...
[#20](https://github.com/CosmicCEO/BoloKit/issues/20) and [#26](https://github.com/CosmicCEO/BoloKit/issues/26) closed via PR #47 (merged to `main`). [#14](https://github.com/CosmicCEO/BoloKit/issues/14) Bonjour advertise/browse/join-via-endpoint — **open**, code landed, live two-peer LAN unverifiable: `NWListener` EINVAL on the physical Mac and Parallels guest, including a bare `swiftc` bind. Do not reopen that investigation.
...
Merged, issues closed, milestone still open (not every v1.4.0 issue done yet):
...
**2026-09-17 dispatch:** [#17](https://github.com/CosmicCEO/BoloKit/issues/17) Game Controller mapping to InputFlags dispatched to Claude Code. Branch `issue-17-game-controller`. Map `GCController` extended gamepad to the 14 game actions in the app target without forking a second keymap in BoloKit (keyboard remap still wins). Set `GCSupportsControllerUserInteraction` to `true` in `Bolo 2026/Info.plist`. Relevant files: `Sources/BoloKit/InputKeymap.swift`, `Bolo 2026/Bolo 2026/GameRenderView.swift`, `Bolo 2026/Bolo 2026/GameSession.swift`, `Bolo 2026/Info.plist`. Verification: `swift test` and `xcodebuild -scheme "Bolo 2026" -only-testing:"Bolo 2026Tests" test`.

**2026-09-17 landed:** [#17](https://github.com/CosmicCEO/BoloKit/issues/17) Game Controller mapping to InputFlags. `InputKeymap.swift`'s `inputFlagsChange(forKeyCode:isDown:bindings:)` split into that keycode-resolution wrapper plus a new `inputFlagsChange(forAction:isDown:)` (all 18 pre-existing `InputKeymapTests` pass unedited). New `GameControllerInput.swift`: pure `controllerDigitalInputs(for:deadzone:)` (fixed left-stick/D-pad/face-button/shoulder scheme onto 10 of the 14 `InputAction`s) and `controllerEdges(from:to:)` take a plain `ControllerSnapshot`, not `GCExtendedGamepad` (no public initializer, so unit-testable at all); `GameControllerInputHandler` is the thin adapter, driven by `valueChangedHandler` + connect/disconnect notifications rather than a polling timer (`GameRenderView` owns no clock, D82). **Disclosed scope gap:** the 4 scroll view-actions have no controller mapping -- a held analog stick can't reproduce the keyboard's per-keypress discrete nudge without a clock this view deliberately lacks. `GameRenderView.applyKeyChange`'s view-action switch extracted to public `performViewAction(_:)`, reused by the controller handler. `GameSession`'s three initializers (single-process/host/join) each construct one `GameControllerInputHandler` after wiring the keyboard closures -- no other `GameSession.swift` path-specific code needed. `GCSupportsControllerUserInteraction` added to `Bolo 2026/Info.plist` (the plist file, not the `INFOPLIST_KEY_` build-setting route -- one edit covers Debug and Release). Swift 6 strict concurrency needed two fixes: `GameControllerInputHandler` subclasses `NSObject` with selector-based `NotificationCenter` (not stored block-observer tokens, which broke `deinit`'s `Sendable` check), and every pure declaration in the new file is explicit `nonisolated` (else the app target's `SWIFT_DEFAULT_ACTOR_ISOLATION = MainActor` makes them uncallable from plain `@Test` funcs). `swift test`: 575 + 230 pass. `xcodebuild -only-testing:"Bolo 2026Tests" test`: 100/100 pass (13 suites; was 86 baseline + 14 new).

Remaining open v1.4.0 work: [#8](https://github.com/CosmicCEO/BoloKit/issues/8) remaining sounds.
