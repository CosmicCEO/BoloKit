...
**2026-09-17 dispatch:** [#8](https://github.com/CosmicCEO/BoloKit/issues/8) Wire remaining procedural sounds dispatched to Claude Code. Branch `issue-8-procedural-sounds`. Wire the remaining designed procedural sounds (`hittank`, `hitterrain`, `hittree`, `mine`, `pillshot`, `build`, `builderdeath`, `sink`, `bubbles`) via BoloKit callback threading (`RunTick`, `ShellTick`, `PillTick`, `BuilderTick`, `TankTick`) and connect them to `SoundPlayer.shared.play(...)` in `GameSession.swift`. Near/far variants stay out until fog-of-war exists. Verification: `swift test` and `xcodebuild -scheme "Bolo 2026" -only-testing:"Bolo 2026Tests" test`.

**2026-09-17 landed:** [#8](https://github.com/CosmicCEO/BoloKit/issues/8) implemented. All 9 remaining sounds now wired: `onHitTank`/`onHitTerrain`/`onHitTree` (`ShellTick.swift`'s `shellTick`/`applyDamage`), `onMine` (`TankLocalTick.swift`'s `plantMine` call sites -- LMINE keydown and lay-while-moving), `onBuild` (`BuilderTick.swift`'s `arriveAtTarget` road/wall/boat/pill/repair/mine success branches), `onBuilderDeath` (`TankLocalTick.swift`'s `killBuilder`, threaded through every mine-detonation/splash-damage/shell-collision path that can kill a builder -- `MineChain.swift`'s `explosionAt`/`superboomAt`, `ShellTick.swift`, `TankTick.swift`), `onSink` (`drown`), `onBubbles` (`tankLocalTick`'s river-drain branch), `onPillShot` (`PillTick.swift`'s `emitPillShell`). All new callback params default to `{}`/`{ _ in }`, so every pre-existing call site is unaffected. Unlike the four sounds wired before this pass (all local-player-only), `onHitTank`/`onBuild`/`onBuilderDeath`/`onSink`/`onPillShot` fire for any connected player -- with fog out of scope (D65), the local player now hears these map-wide, disclosed in `SoundPlayer.swift`'s own header. Join-mode client still doesn't run `runTick` (D116) so these stay unwired there, matching the existing `onTankShot`/`onTreeHarvest` precedent. `swift test`: 230 + 594 pass (824 BoloKitTests/DifferentialTests total, +19 new tests over the 805 baseline; one pre-existing flaky timing test, `hostGameEngineSubmitPauseResumeServerTogglesPauseState`, isolated flake, passes standalone). Advisor review caught and fixed three real bugs before landing: `onPillShot` was firing once per tied shell target instead of once per shot (moved to `pillTick`'s counter-reset site); `applyDamage`'s hit-terrain/hit-tree hooks were skipped on pill/base hits, diverging from `recvsrdamage()`'s function-scope placement (moved above the pill/base branches); `onSink` was firing unconditionally instead of only inside `drown()`'s own dead/respawn-counter guard (moved inside it, matching client.c:5581-5583). `xcodebuild -scheme "Bolo 2026" -only-testing:"Bolo 2026Tests" test`: 86/86 pass.
