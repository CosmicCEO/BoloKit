...
[#3](https://github.com/CosmicCEO/BoloKit/issues/3) Join-side connection lag tint -- closed the disclosed gap in `GameSession.connectionAge(for:)`: the join path now derives an age from `UDPSession`'s own per-peer freshness table (new `lastUpdate(for:)` accessor) minus this session's `localSeq`, the same `currentTick - lastUpdate` shape the host path already gets from `HostSessionTable.allTicksSinceLastUpdate`. `PlayerStatusView.staleness(forPlayer:)` needed no changes -- it already just reacts to any non-nil age. Branch `claude/plan-next-issue-5u8ew6`. New test: `udpSessionLastUpdateReflectsCallersOwnSeqAtApplyTime` in `UDPSessionTests.swift`. **Written and reasoned through in a Linux container with no Swift/Xcode toolchain available -- not run through `swift test`/`xcodebuild` yet.** Needs a real test pass (expect ~803 SwiftPM: 575 + 228) before this line can drop the caveat or the GitHub issue closes.
