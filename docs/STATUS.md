...
**Current tagged drop:** `v1.5.0`. [GitHub release](https://github.com/CosmicCEO/BoloKit/releases/tag/v1.5.0). **`v1.4.0`–`v1.2.0`, `v1.1.0`, and `v1.0.0` shipped.** **`v1.5.1 — Live play fixes`** (milestone [19](https://github.com/CosmicCEO/BoloKit/milestone/19)) has **no open issues left** and its code (PRs [#117](https://github.com/CosmicCEO/BoloKit/pull/117), [#121](https://github.com/CosmicCEO/BoloKit/pull/121), [#123](https://github.com/CosmicCEO/BoloKit/pull/123)) is merged to `main` — but the milestone is **not tagged yet**: #114's fix (below) was verified by unit test and code read only, with no live two-Mac hardware available. Draft release notes: `docs/RELEASE_NOTES_v1.5.1.md`.
...
**[#114](https://github.com/CosmicCEO/BoloKit/issues/114) fixed and merged (2026-09-22, PR [#123](https://github.com/CosmicCEO/BoloKit/pull/123)):** `headingColumn(shell.dir)` returns a 16-heading bucket, but the procedurally-generated sprite sheet only populated 6 of those 16 `SHELL` cells (`GlyphSource.swift`'s `.shell(frame:)` mistook them for animation-growth frames, not headings) — the other 10 buckets (`0x66-0x6f`) were an intentional transparent gap. Since a shell's `dir` never changes after firing, a shell fired toward one of those 10 headings was invisible for its whole flight, exactly the angle-dependent behavior the 2026-09-22 run reported. Fix: widened the shell range to all 16 headings and gave shells a constant-size directional streak instead of a growing circle; `headingColumn` moved to `PhysicsOps.swift` (public) for testability. New tests in `BoloGlyphsTests.swift` sweep every heading. **v1.5.1 now has no open issues** — **no live two-Mac hardware was available to confirm this fix**, so it's verified by unit test/code read only; a real heading-sweep check is still owed before the tag.

**[#105](https://github.com/CosmicCEO/BoloKit/issues/105) fixed and merged (2026-09-22, PR [#121](https://github.com/CosmicCEO/BoloKit/pull/121)):** a host-simulated guest's key-down mine landed on the tile it already occupied, and `remoteLastTankPosition` was still one tile behind, so the next tick detonated it. `9a58453` (now on `main`) stamps the tank's current position when that drop is on its own tile. A snap onto a mine that was already there still detonates. Tests: `dispatchDropMineUnderAHostSimulatedTankDoesNotDetonateOnTheNextTick`, `hostSimulatedTankAlreadyOnAMineStillDetonatesWhenTheEntryStampIsBehind` — both pass, along with the full fog/calcvis suite (52/52) run before the merge; full suite still not run. Closed per 2026-09-22 triage direction even though the 2026-09-22 two-Mac run above didn't re-exercise the original continuous-lay-while-moving repro scenario — reopen if it resurfaces. Pull requests [#95](https://github.com/CosmicCEO/BoloKit/pull/95) and [#109](https://github.com/CosmicCEO/BoloKit/pull/109) were closed as superseded by #117; [#71](https://github.com/CosmicCEO/BoloKit/issues/71) (stale README/hiddenMines comment) also merged and closed in the same PR #121.
...
**Next step:** get a live two-Mac session (fire shells at a range of headings, both Hidden Mines on/off) confirming #114's fix and, ideally, re-exercising #105's original continuous-lay-while-moving scenario. Run full `swift test` against `main`. Then cut the `v1.5.1` tag and update this file's "Shipped" section. v1.5.2 then picks up #113, #120 (its focus), #89, #93, #100, #101, #87, #118.
