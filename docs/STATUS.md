...
**Current tagged drop:** `v1.6.6`. [GitHub release](https://github.com/CosmicCEO/BoloKit/releases/tag/v1.6.6), merged via [PR #154](https://github.com/CosmicCEO/BoloKit/pull/154) 2026-09-24. Fifth `1.6.x` theme, visibility parity -- forest concealment (#153), the missed fifth theme on top of the man/tank/boat/sound arc. Release notes: `docs/RELEASE_NOTES_v1.6.6.md`. **`v1.6.5`–`v1.2.0`, `v1.1.0`, and `v1.0.0` shipped.** `v1.6.5`: [GitHub release](https://github.com/CosmicCEO/BoloKit/releases/tag/v1.6.5), merged via [PR #151](https://github.com/CosmicCEO/BoloKit/pull/151) 2026-09-24, bundles the visual-polish milestone (`v1.6.1`, #137/#140/#110) and the four-theme parity arc (man/tank/boat/sound, milestones 23-26) plus #139's host-engine crash fix. Release notes: `docs/RELEASE_NOTES_v1.6.5.md`. `v1.6.0`: milestone [4](https://github.com/CosmicCEO/BoloKit/milestone/4) closed 2026-09-23, #25 closed, code merged to `main` (PRs [#128](https://github.com/CosmicCEO/BoloKit/pull/128), [#141](https://github.com/CosmicCEO/BoloKit/pull/141)), Metal renderer plus the guest desync fixes it surfaced both live-confirmed on real two-Mac hardware. Release notes: `docs/RELEASE_NOTES_v1.6.0.md`.
...
**2026-09-24, same day:** [#157](https://github.com/CosmicCEO/BoloKit/issues/157) -- host/client login UX. Host had no player-name field at all (`startHosting()` read `UserDefaults` directly); Join's player-name field was plain `@State`, seeded once, edits never persisted. Both now bind `@AppStorage("GSPlayerNameString")` directly, one shared identity matching the oracle's single `playerNameString` ivar. Added an optional Game Name field (host-only, new relative to the oracle -- it has no such concept, joiners there see host player name + map filename) and a "Pause on Player Exit" toggle surfacing `GameState.pauseOnPlayerExit` (already simulated, never exposed in either the oracle's own GUI or this port's). `hiddenMines` default flipped `false` -> `true` (deliberate product decision, diverges from the oracle's own default-off). Verified: `swift build`/`xcodebuild build` clean, `swift test --filter BoloKitTests` 649/649, `xcodebuild test` (real `HostListener` networking) 150/151 (1 pre-existing unrelated flake, confirmed on isolated retry). `DifferentialTests` under bare `swift test` hung repeatedly this session -- a known environmental instability with real-socket tests in that harness specifically (see `project_xcode_toolchain_hang` memory), not exercised by this change; equivalent real-networking coverage passed under `xcodebuild test`.
