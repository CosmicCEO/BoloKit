...
**Current tagged drop:** `v1.5.0`. [GitHub release](https://github.com/CosmicCEO/BoloKit/releases/tag/v1.5.0). **`v1.4.0`–`v1.2.0`, `v1.1.0`, and `v1.0.0` shipped.** **`v1.5.1 — Live play fixes`** (milestone [19](https://github.com/CosmicCEO/BoloKit/milestone/19)) is in flight: its code (PRs [#117](https://github.com/CosmicCEO/BoloKit/pull/117), [#121](https://github.com/CosmicCEO/BoloKit/pull/121)) merged to `main`, but the milestone stays open and untagged until its last real issue (#114) closes — see "In flight" below. Draft release notes: `docs/RELEASE_NOTES_v1.5.1.md`.
...
**Still genuinely open in v1.5.1:** just [#114](https://github.com/CosmicCEO/BoloKit/issues/114), shells not always visible for their whole flight, now with angle data pointing at a possible heading-column mapping bug. Fix it, confirm with `swift test` plus a live two-Mac run, then cut the `v1.5.1` tag from `docs/RELEASE_NOTES_v1.5.1.md`.
...
**Next step:** fix [#114](https://github.com/CosmicCEO/BoloKit/issues/114) (shells), the one issue still open in v1.5.1 — the angle data from the 2026-09-22 run points at a possible `headingColumn(shell.dir)`/shell sprite-sheet mapping bug rather than a pure fog-fade issue. Confirm with `swift test` plus a live two-Mac run, then cut the `v1.5.1` tag and update this file's "Shipped" section. v1.5.2 then picks up #113, #120 (its focus), #89, #93, #100, #101, #87, #118.
